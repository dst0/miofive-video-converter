...
| SEC-004 | High | Fixed after follow-up | Initial exclusive reservation was insufficient because FFmpeg reopened the public name. Export now encodes in private staging, then publishes complete bytes using atomic no-clobber hard links, suffixes on collision and never deletes public output on late disconnect. Unsupported filesystems fail before encoding. Concurrency/input/range bounds remain (`export-output.js`, `index.js`). |
...
| SEC-008 | Medium | Fixed | Browser responses lacked a maintained security-header policy. Helmet now sets a restrictive CSP and related headers, Express disclosure is disabled, JSON is strict and capped at 64 KiB, and Tauri has a matching non-null CSP (`index.js`, `src-tauri/tauri.conf.json`). |
| SEC-009 | Medium | Fixed | FFmpeg source/version and CI actions were mutable or insufficiently governed. The build uses checksum-pinned FFmpeg 9.0.1 and x264 sources, disables networking/nonfree output, verifies license text, pins Actions to commits, and applies least permissions/timeouts (`scripts/ffmpeg-build-config.js`, `scripts/build-ffmpeg-macos-arm64.sh`, `.github/workflows/node.js.yml`, `.github/workflows/deploy-demo.yml`). |
...
| SEC-012 | High | Fixed | Only browsing/video used an ad hoc quota. Maintained rate-limit middleware now bounds all private control routes before body parsing and expensive work, separately from media bursts; spoofed forwarding headers cannot add quota. |
| SEC-013 | High | Fixed | Cached manifest validation followed by pathname truncation allowed a replacement symlink to redirect writes. Read through a regular no-follow descriptor, then flush and atomically publish an exclusively created sibling. Failed writes retain the previous record. |
| SEC-014 | Medium | Fixed | A frontend test wrote into a predictable temporary folder. Use `mkdtemp` instead of timestamps/Math.random plus recursive creation. |

## Remaining risks and accepted boundaries

### RISK-000 — Trusted host and destination assumptions

Loopback/browser metadata checks are not OS-user/process authentication. Native local clients can omit browser headers and use the API's intended arbitrary local paths. The host/local clients must be trusted; this is not an isolation boundary for a shared or hostile computer. Normal-mode `path.resolve` is normalization, not traversal prevention; demo containment additionally uses canonical paths. Output and build parents must remain trusted/stable during work. Hard-link-free destinations fail closed; power loss or forced kill may leave private staging for inspection. See `docs/architecture.md` and `docs/codeql-triage.md` for exact scope, evidence and per-alert decisions.
