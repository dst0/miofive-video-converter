...
- **Verification:**
  ...
  - `cargo audit --file src-tauri/Cargo.lock`: Exited 0 with 0 vulnerabilities and 17 allowed warnings. Truthfully confirmed the known Linux-target `glib` advisory (`RUSTSEC-2024-0429`: unsoundness in `VariantStrIter`, in the `webkit2gtk`/`gtk` tree for Linux targets), which does not affect the macOS desktop application or Node runtime and is not suppressed or dismissed.
