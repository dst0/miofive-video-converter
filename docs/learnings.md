...
### 2026-09-08 — Consolidated dependency integration via exact patch union avoids resolver drift

- **Status:** Corrected
- **Correction (2026-09-08):** Following publication, root review identified two overstatements in the initial draft. Authoritative direct diff evidence established collateral Cargo resolver changes to `windows-core` and `windows-sys` references and removal/deduplication of Windows lock entries; do not claim specific `windows-targets` or `windows_x86_64_gnullvm` names unless currently reproducible. Also, sequential merging would not technically force rebases: it would either accept CI run against the old shared base or require refreshed/rebased heads and rerun CI to prove the combined state. The failed attempt wording is also corrected to blame the observed `cargo update` resolver drift; `npm install` did not cause Cargo drift.
- **Correction (2026-09-08):** An independent live audit on exact head `e146756` corrected the stale warning count of 17 (16 unmaintained, 1 unsoundness) to the authoritative 7 allowed warnings (exit 0): 6 unmaintained advisories (`proc-macro-error` plus five `unic` crates: `unic-char-property`, `unic-char-range`, `unic-common`, `unic-ucd-ident`, `unic-ucd-version`) and 1 unsound `glib` `RUSTSEC-2024-0429` warning. Prior correction transparency is preserved while updating verification claims to match authoritative live execution.
- **Task/context:** Integrating post-merge dependency updates following the merge of PR #45 at `7670dfd8be6dccb73d62d811fb0f25d5b508c563` into `main`, covering five active Dependabot PRs: #46 (`libc` 0.2.189), #47 (`tauri-plugin-shell` 2.3.6), #48 (`tauri-build` 2.6.3), #49 (`serde_json` 1.0.151) in `src-tauri/Cargo.lock`, and #50 (`@playwright/test` 1.63.0, `eslint` 10.10.0) in `package.json` and `package-lock.json`.
- **Unexpected observation or failure:** An initial attempt to regenerate dependency updates using `cargo update` triggered Cargo's cross-platform dependency resolver on macOS, resulting in collateral resolver drift across optional and platform-specific entries (specifically altering `windows-core`/`windows-sys` references and removing/deduplicating Windows lock entries outside the reviewed Dependabot PR diffs). Furthermore, treating post-merge Dependabot branches as stale targets would discard valid, reviewed isolated changes that were already freshly rebased by Dependabot onto the exact post-#45 merge commit `7670dfd8be6dccb73d62d811fb0f25d5b508c563`.
- **Evidence:** Running `cargo update` produced diffs in `src-tauri/Cargo.lock` touching `windows-core`/`windows-sys` references and removing/deduplicating Windows lock entries, diverging from the clean single-dependency bumps proposed by Dependabot. Conversely, inspecting the five Dependabot PR heads (`9047e4901b095b9caf43cb62a8ad797cdd9f45da`, `6524560446312c8d0ac921c1a0e6b6ce4a528090`, `0375376251ed6ff857683b01765c9ec9e9eb854d`, `0bd962bccc2b87c384508df4021fea5cb117f50d`, and `a1e5741d4be2bd01d6546975452494f447e7e3a8`) via `git log` and `git show` proved each commit had the exact parent `7670dfd8be6dccb73d62d811fb0f25d5b508c563` and contained only the reviewed target package updates.
- **Approaches tried:**
  - **Attempt:** Run `cargo update` locally to regenerate Cargo lockfile updates.
    - **Outcome:** Did not work
    - **Why:** Cargo's resolver modified collateral cross-platform (Windows) dependency lock entries (`windows-core`/`windows-sys` references and removed/deduplicated Windows lock entries) on the local macOS workstation, introducing unreviewed resolver drift. This observed drift was caused by `cargo update` (not `npm install`). This attempt was interrupted and reverted.
  - **Attempt:** Sequentially merge each Dependabot PR (#46 through #50) one by one through GitHub.
    - **Outcome:** Considered and rejected
    - **Why:** Sequential merging would not technically force rebases: it would either accept CI run against the old shared base (leaving the combined multi-package state unproven until all were merged) or require refreshed/rebased heads and rerun CI to prove the combined state.
  - **Attempt:** Consolidate the five approved Dependabot heads by applying the exact patch union using sequential `git cherry-pick --no-commit` on the clean worktree at `7670dfd8be6dccb73d62d811fb0f25d5b508c563`, followed by combined gate validation.
    - **Outcome:** Worked
    - **Why:** Each Dependabot branch was already an isolated single-commit patch cleanly rebased on `7670dfd`. Cherry-picking the exact commit sequence (`9047e49`, `6524560`, `0375376`, `0bd962b`, `a1e5741`) produced the exact union of reviewed changes with zero conflicts, zero unrelated resolver drift, and identical byte-level lockfile changes.
- **Root cause:** `cargo update` invokes resolution algorithms that can update transient, optional, or cross-platform lockfile entries across the broader dependency graph, whereas Dependabot's isolated single-package commits isolate changes to the exact target packages.
- **Resolution:** Applied the exact union of the five reviewed Dependabot commits via `git cherry-pick --no-commit`. Used `npm ci --ignore-scripts` to install the exact npm lockfile without modifying it, and verified that lockfile SHA-256 hashes remained identical before and after installation. Downloaded updated Playwright browser binaries via `npx playwright install chromium` following the `@playwright/test` 1.63.0 upgrade. Ran combined validation across the consolidated dependency state. Left `.github/dependabot.yml` untouched, introduced no runtime features or `.mjs` files, and preserved transparent reporting of the Linux-only `glib` advisory.
- **Verification:**
  - Lockfile integrity: SHA-256 hashes of `package.json`, `package-lock.json`, and `src-tauri/Cargo.lock` proved 100% identical before and after `npm ci --ignore-scripts`, and the diff matched the exact union of the five Dependabot commits with zero extraneous lines.
  - `npm audit --audit-level=high`: Exited 0 with 0 vulnerabilities found.
  - `cargo audit --file src-tauri/Cargo.lock`: Exited 0 with 0 vulnerabilities and 7 allowed warnings (6 unmaintained advisories: `proc-macro-error` plus five `unic` crates, and 1 unsoundness warning: `glib` `RUSTSEC-2024-0429`). Truthfully confirmed the known Linux-target `glib` advisory (`RUSTSEC-2024-0429`: unsoundness in `VariantStrIter`, in the `webkit2gtk`/`gtk` tree for Linux targets), which does not affect the macOS desktop application or Node runtime and is not suppressed or dismissed.
  - Full prepush suite (`npm run prepush`): Passed cleanly with exit 0 (ESLint, 79 unit tests, license metadata checks, 179 Playwright E2E tests with single worker and 0 retries, Rust formatting, Clippy with warnings denied, and 1 Rust test).
  - `git diff --check`: Clean exit with zero whitespace or formatting errors.
