...
- **Unexpected observation or failure:** An initial attempt to regenerate dependency updates using local package manager commands (`cargo update -p ...`) triggered Cargo's cross-platform dependency resolver on macOS, resulting in collateral resolver drift across optional and platform-specific entries (specifically updating Windows lockfile targets such as `windows-targets` and `windows_x86_64_gnullvm` that were outside the reviewed Dependabot PR diffs). Furthermore, treating post-merge Dependabot branches as stale targets would discard valid, reviewed isolated changes that were already freshly rebased by Dependabot onto the exact post-#45 merge commit `7670dfd8be6dccb73d62d811fb0f25d5b508c563`.
- **Evidence:** Running `cargo update` produced diffs in `src-tauri/Cargo.lock` touching unrelated `windows_x86_64_*` entries, diverging from the clean single-dependency bumps proposed by Dependabot. Conversely, inspecting the five Dependabot PR heads (`9047e4901b095b9caf43cb62a8ad797cdd9f45da`, `6524560446312c8d0ac921c1a0e6b6ce4a528090`, `0375376251ed6ff857683b01765c9ec9e9eb854d`, `0bd962bccc2b87c384508df4021fea5cb117f50d`, and `a1e5741d4be2bd01d6546975452494f447e7e3a8`) via `git log` and `git show` proved each commit had the exact parent `7670dfd8be6dccb73d62d811fb0f25d5b508c563` and contained only the reviewed target package updates.
- **Approaches tried:**
  - **Attempt:** Run `cargo update` and `npm install` locally to bring dependencies up to date.
    - **Outcome:** Did not work
    - **Why:** Cargo's resolver modified unrelated cross-platform (Windows) dependency lock entries on the local macOS workstation, introducing unreviewed resolver drift. This attempt was interrupted and reverted.
  - **Attempt:** Sequentially merge each Dependabot PR (#46 through #50) one by one through GitHub.
    - **Outcome:** Considered and rejected
    - **Why:** Each merge invalidates the base of the remaining 4 PRs, forcing Dependabot to sequentially rebase and rerun CI 5 times, causing unnecessary pipeline churn and delay.
...
- **Root cause:** Package manager update commands invoke heuristic resolution algorithms that can update transient, optional, or cross-platform lockfile entries across the broader dependency graph, whereas Dependabot's isolated single-package commits isolate changes to the exact target packages.
...
- **Prevention/follow-up:** When consolidating multiple concurrent Dependabot PRs based on the same parent commit, do not run `cargo update` or `npm install` to generate the diff. Instead, treat immediate post-merge Dependabot branches as current valid proposals and apply their exact reviewed commit patches via `git cherry-pick --no-commit`. Validate lockfile stability by verifying hashes across clean `npm ci --ignore-scripts`. Always report platform-specific security advisories (such as Linux-only `glib`) transparently rather than adding broad suppressions or mutating `.github/dependabot.yml`.
