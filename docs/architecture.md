...
Each assigned source also owns error listeners for both the video and its child source element, retained after initial readiness. Only a current active source can cancel pending readiness/seeks, pause playback and display recovery guidance. Cross-clip seeks have the same bounded failure path. Source changes and Back remove obsolete listeners; inactive preload failures do not interrupt the active clip. A queued `timeupdate` from pausing is not evidence of recovery and must not clear the message. Selecting another clip or successful current readiness/playback clears it; manual Next preserves paused intent.
...
Local installation validates the known Tauri bundle layout before copying and again before promotion: `Contents/MacOS/miofive-video-converter`, `miofive-server`, and nested `Contents/Resources/resources` assets, notices and executable media tools. Existing media manifests use the canonical hash/pin validator; explicit operator-tool bundles may omit a source-build manifest as documented by their generated notice. This is a completeness check, not Mach-O, signature or notarization certification. Exclusive staging creation establishes cleanup ownership; a colliding staging directory is never removed. The old installation remains until staging passes, and a failed promotion attempts restoration from its retained backup. Destination parents are trusted and concurrent installers are unsupported.
